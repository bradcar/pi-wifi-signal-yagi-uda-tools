...
# Import the mock environment function from your cardiod_test_data_generator.py utility
from cardiod_test_data_generator import measured_signal_strength
...
def display_radar_ssd(circle_bbox, draw, angle: float, rssi: float):
    """
    Draws a white radar circle with a black center indicator dot.
    Sweeps an active line proportional to the provided mock signal strength.
    -20dBm: Full length vector line extending out to the circle border.
    -80dBm: Zero length vector line hidden within the center point.
    -99dBm (Dead-Zone): Falls below floor calculation bounds, showing as 0 length.
    """
    ...
    # Clamp the target RSSI to our specific mapping bounds (-20 to -80)
    current_rssi = rssi
    ...
    # -20 dBm -> 1.0 (Full length) | -80 dBm or lower -> 0.0 (Zero length)
    ...
    # Draw vector line out from center point if length allows
    if line_length > 0:
        draw.line((center_x, center_y, target_x, target_y), fill=0)

    # Draw the black center point dot over the axis lines
...
def main():
    ...
    try:
        ...
        while True:
            # 1. Get Live Environment Info for text display box
            try:
                ssid = get_ssid()
            ...
            # 2. Get Test Data for the radar graphic sweep vector box
            # Pulls directly from your cardiod_test_data_generator.py MOCK_SIGNAL_ARRAY configuration
            ...
            if ssd1305_detected:
                ...
                # Left side text tracks the live wlan0 data
                display_text_ssd(draw, font, rssi, ssid, tx_rate, heading, connected=is_connected)

                # Right side radar graphic tracks the sweeping test data stream
                display_radar_ssd(circle_bbox, draw, sweep_angle, mock_rssi)

                display.image(image)
                display.show()

            # Increment the sweep angle by exactly 1 degree for the next pass
            sweep_angle = (sweep_angle + 1) % 360
